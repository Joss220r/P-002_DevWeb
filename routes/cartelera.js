 ...
 * tags:
 *   name: Cartelera
 *   description: API para gestionar la cartelera de películas
 */
...
router.get('/', async (req, res) => {
    try {
        ...
        const result = await pool.request().query('SELECT imdbID, Titulo, Anio, Tipo, Poster, Estado, Descripcion, Ubicacion FROM peliculas1035');
...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        await pool.request()
            ...
            .query('INSERT INTO peliculas1035 (imdbID, Titulo, Anio, Tipo, Poster, Estado, Descripcion, Ubicacion) VALUES (@imdbID, @Titulo, @Anio, @Tipo, @Poster, @Estado, @Descripcion, @Ubicacion)');
...
router.put('/:imdbID', async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request()
            ...
            .input('Descripcion', sql.NVarChar(sql.MAX), Descripcion)
            .input('Ubicacion', sql.NVarChar(100), Ubicacion)
            .query('UPDATE peliculas1035 SET Titulo = @Titulo, Anio = @Anio, Tipo = @Tipo, Poster = @Poster, Estado = @Estado, Descripcion = @Descripcion, Ubicacion = @Ubicacion WHERE imdbID = @imdbID');
